...
def process_validation_metrics(
    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                if var_vals[0] is None or isinstance(var_vals[0], (str, list)):
                    continue

                data_src2uid2var2metric[data_source][uid][var_name] = {
                    f"mean@{len(var_vals)}": np.mean(var_vals)
                }
    ...
    print(f"Data source to variable to metric values: {data_src2var2metric2val}")
