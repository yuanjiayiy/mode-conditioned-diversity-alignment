...
class RewardFnActor:
    """
    Lightweight Ray actor that keeps a reward function alive on dedicated resources.
    """

    def __init__(self, config: DictConfig | dict, tokenizer):
        if not isinstance(config, DictConfig):
            config = OmegaConf.create(config)
        self.config = config
        self.tokenizer = tokenizer
        self.reward_fn = load_reward_manager(
            self.config, self.tokenizer, num_examine=0, **self.config.reward_model.get("reward_kwargs", {})
        )
